...
or only missing-description jobs that still lack a stable detail-page source. A new tracked view
combines interested, contacted, applied, and interviewed jobs in one list while preserving each
platform record.
...
controls are visible. Job Boardwalk reuses an existing tab only when it can inspect the page and
confirm that the tab remains on the platform's login route. It leaves unreadable or unclassified
pages unchanged, including pages that may be showing verification, and uses an available blank tab
or a new tab for login instead. It also distinguishes a navigation timeout from what it can still
observe on the resulting page, helping the agent reconcile the visible result before choosing a
safe next step instead of automatically repeating an action whose outcome is uncertain.
