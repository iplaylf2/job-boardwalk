...
const browserTools = [
  ...
  defineBrowserTool({
    ...
    description:
      "读取当前招聘平台标签页中已加载的岗位卡片，返回有界、去重的页面证据；岗位跟进采集器拥有的页面不属于此工具的读取范围，调用会失败。此工具不会导航、滚动、点击或持久化岗位，但同一次读取可能刷新平台适配器能够明确判定的访问观察，并将其加入 Browser Session 状态上报。独立的被动采集流程会定期提交所有合格页面中的岗位卡片；当前求职方向只提供自动打开的研究起点，不限制其他合格页面参与采集。",
